    ...
    useEffect(() => {
        const loadInitialData = async () => {
            try {
                ...
                setSites(sitesData);
                // Fetch recent alerts on initial load
                handleSearch();
            } catch (error) {
            ...
            <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4 items-end">
                    <div>
                        ...
                        <input type="date" name="startDate" value={filters.startDate} onChange={handleFilterChange} className="w-full bg-gray-700 border-gray-600 rounded-md mt-1"/>
                    </div>
                    <div>
                        <label className="text-sm text-gray-400">End Date</label>
                        <input type="date" name="endDate" value={filters.endDate} onChange={handleFilterChange} className="w-full bg-gray-700 border-gray-600 rounded-md mt-1"/>
                    </div>
                    <div>
                        <label className="text-sm text-gray-400">Site</label>
                        <select name="siteId" value={filters.siteId} onChange={handleFilterChange} className="w-full bg-gray-700 border-gray-600 rounded-md mt-1">
                            <option value="">All Sites</option>
                            {sites.map(site => <option key={site._id} value={site._id}>{site.name}</option>)}
                        </select>
                    </div>
                    <button onClick={handleSearch} className="px-4 py-2 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 h-10">Search</button>
                </div>
                    ...
                    <table className="min-w-full">
                        ...
                        <tbody className="bg-gray-800 divide-y divide-gray-700">
                            {isLoading ? (
                                ...
                                <tr key={alert._id} onClick={() => setSelectedAlert(alert)} className="hover:bg-gray-700/50 cursor-pointer">
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{alert.siteProfile?.name}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{alert.originalData.camera?.name}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{alert.originalData.event_type}</td>
                ...
                <div className="p-6 flex-1 overflow-y-auto">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <ul className="mt-2 space-y-2 text-gray-300 bg-gray-700/50 p-3 rounded-md">
                     ...
                     <div className="mt-6">
                        <div className="flex justify-between items-center mb-2">
                            ...
                            <button onClick={() => setShowRawData(!showRawData)} className="text-xs text-gray-400 hover:text-white">
                                {showRawData ? 'Hide Raw Data' : 'Show Raw Data'}
                            </button>
                        </div>
                        <div className="bg-gray-900/50 rounded-lg p-3 space-y-3 h-48 overflow-y-auto">
                            {(alert.notes || []).length > 0 ? alert.notes.map((note, index) => (
                                <div key={index} className="text-sm border-b border-gray-700 pb-2">
                                    ...
                                    <p className="text-xs text-gray-500 text-right">- {note.username} at {new Date(note.timestamp).toLocaleString()}</p>
                                </div>
                            )) : <p className="text-sm text-gray-500">No notes for this event.</p>}
                        </div>
                        {showRawData && (
                            <div className="mt-4">
                               <h4 className="text-sm font-semibold text-gray-400">Raw Alert Data</h4>
                               <pre className="text-xs bg-black p-2 rounded-md overflow-x-auto text-gray-300 max-h-48">
