...
import { ChevronDown, ChevronRight, Pen, Save, X, Moon, Sun } from 'lucide-react';
import { api } from '../api';
...
function DeviceManagementPage() {
    ...
    return (
        <div className="space-y-8 text-brand-300">
            <h1 className="text-3xl font-bold text-white">Device & Site Management</h1>
            <div className="bg-brand-800 p-6 rounded-lg shadow-lg">
                <p className="text-brand-400 mb-6">Manage site profiles and control monitoring status for all cameras detected from the Turing API.</p>
                {isLoading ? <p>Loading devices...</p> : (
                    <div className="space-y-2">
                        {devices.map(site => {
                            const allMonitored = site.cameras.length > 0 && site.cameras.every(c => c.isMonitored);
                            return (
                                <div key={site._id} className="bg-brand-700/60 rounded-lg">
                                    <div className="w-full flex justify-between items-center p-4 text-left">
                                        <button onClick={() => toggleSite(site._id)} className="flex items-center flex-1">
                                            ...
                                            <p className="font-semibold text-lg text-white">{site.name || `Site ID: ${site._id}`}</p>
                                            {!site.isConfigured && (
                                                <span className="ml-4 text-xs bg-yellow-600 text-white font-bold py-1 px-2 rounded-full">Needs Configuration</span>
                                            )}
                                        </button>
                                        <div className="flex items-center space-x-4">
                                            <button onClick={() => setEditingSite(site)} className="flex items-center px-3 py-1 text-sm bg-brand-600 hover:bg-brand-500 rounded-md text-white">
                                                <Pen size={14} className="mr-2"/> Edit Profile
                                            </button>
                                            <div className="flex items-center space-x-2">
                                                <label className="text-sm text-brand-300">Monitor All</label>
                                                <input type="checkbox" checked={allMonitored} onChange={(e) => handleMonitorAllToggle(site._id, e.target.checked)} className="h-4 w-4 rounded bg-brand-900 border-brand-600 text-accent focus:ring-accent"/>
                                            </div>
                                        </div>
                                    </div>
                                    {openSites[site._id] && (
                                        <div className="border-t border-brand-600 p-4">
                                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                                {site.cameras.map(camera => {
                                                    const isSleeping = camera.isSleeping && new Date(camera.sleepExpiresAt) > new Date();
                                                    return (
                                                        <div key={camera.id} className={`p-3 rounded-md flex justify-between items-center ${isSleeping ? 'bg-indigo-900/50' : 'bg-brand-800'}`}>
                                                            <div>
                                                            ...
                                                            <div className="flex items-center space-x-3">
                                                                {isSleeping ? (
                                                                    ...
                                                                    <button onClick={() => setSleepingCamera(camera)} className="p-1 text-brand-400 hover:text-white" title="Put camera to sleep">
                                                                        <Moon size={16} />
                                                                    </button>
                                                                )}
                                                                <label className="relative inline-flex items-center cursor-pointer">
                                                                    ...
                                                                    <div className="w-11 h-6 bg-brand-600 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-brand-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-accent"></div>
                                                                </label>
                                                            </div>
                                                        </div>
                                                    )
...
function CameraSleepModal({ camera, onClose, onConfirm }) {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
            <div className="bg-brand-800 rounded-lg shadow-2xl w-full max-w-md">
                <div className="p-5 border-b border-brand-700 flex justify-between items-center">
                    <h2 className="text-xl font-bold text-white">Put Camera to Sleep</h2>
                    <button onClick={onClose} className="text-brand-400 hover:text-white"><X size={24}/></button>
                </div>
                <div className="p-6 space-y-4">
                    <p className="text-brand-300">Temporarily pause monitoring for <span className="font-semibold text-white">{camera.name}</span>.</p>
                    <div>
                        <label className="block text-sm font-medium text-brand-400 mb-1">Duration (in hours)</label>
                        <input type="number" min="1" value={hours} onChange={(e) => setHours(Number(e.target.value))} className="w-full bg-brand-700 border-brand-600 rounded-md text-white"/>
                    </div>
                </div>
...
function SiteEditModal({ site, onClose, onSave }) {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
            <div className="bg-brand-800 rounded-lg shadow-2xl w-full max-w-lg">
                <div className="p-5 border-b border-brand-700 flex justify-between items-center">
                    <h2 className="text-xl font-bold text-white">Edit Site: {site.name}</h2>
                    <button onClick={onClose} className="text-brand-400 hover:text-white"><X size={24}/></button>
                </div>
                <div className="p-6 space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-brand-400 mb-1">Account Number</label>
                        <input name="account_number" value={formData.account_number || ''} onChange={handleChange} className="w-full bg-brand-700 border-brand-600 rounded-md text-white"/>
                    </div>
                     <div>
                        <label className="block text-sm font-medium text-brand-400 mb-1">District</label>
                        <input name="district" value={formData.district || ''} onChange={handleChange} className="w-full bg-brand-700 border-brand-600 rounded-md text-white"/>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-brand-400 mb-1">Pertinent Information</label>
                        <textarea name="pertinent_info" value={formData.pertinent_info || ''} onChange={handleChange} rows="4" className="w-full bg-brand-700 border-brand-600 rounded-md text-white"></textarea>
                    </div>
                </div>
                <div className="bg-brand-700/50 px-6 py-4 flex justify-end">
                   <button onClick={() => onSave(formData)} className="flex items-center px-4 py-2 bg-accent text-brand-900 font-semibold rounded-lg hover:bg-accent-hover">
